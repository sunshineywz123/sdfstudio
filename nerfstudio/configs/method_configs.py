...
method_configs["geo-neus"] = Config(
    ...
    trainer=TrainerConfig(
        steps_per_eval_image=200000,
        steps_per_eval_batch=200000,
        steps_per_save=200,
        steps_per_eval_all_images=1000000,  # set to a very large model so we don't eval with all images
        max_num_iterations=200000,
        mixed_precision=False,
...
method_configs["nerfacto"] = Config(
    ...
    trainer=TrainerConfig(
        ...
        steps_per_save=200, max_num_iterations=30000, mixed_precision=True
    ),
    pipeline=VanillaPipelineConfig(
        datamanager=VanillaDataManagerConfig(
...
method_configs["semantic-nerfw"] = Config(
    ...
    trainer=TrainerConfig(
        steps_per_eval_batch=500, steps_per_save=200, max_num_iterations=30000, mixed_precision=True
    ),
    pipeline=VanillaPipelineConfig(
